...
from general_utils import save_torch_model
from HEB_DB import featch_triplet_aux_dataloader
import pickle
import os


"""
   Compute the triplet loss beetween 3 vectors
    
    Args:
        anchor (torch.Tensor): Input waveform
        positive (torch.Tensor): a waveform we are trying to pull towards anchor
        negetive (torch.Tensor): a waveform we are trying to get away from anchor
...
val_aux = featch_triplet_aux_dataloader(folder_val,batch_size=16)
train_aux = featch_triplet_aux_dataloader(folder_train,batch_size=16)
# Build the model
model = DSCNN()
